...
1. Windows 底部窗口、任务栏避让、置顶切换和屏幕占用；基础方案已验证，最终窗口行为延后到核心游戏体验稳定后继续处理。
...
2. 实现点击移动与基础情境交互。
3. 实现单杯订单循环与测试饮用计时。
4. 加入占位 NPC，并验证顾客占座与玩家优先规则。
5. 加入播放器接口、本地设置和性能检查。
6. 核心游戏体验稳定后继续验证 Windows 停靠、尺寸与置顶行为，再购买成套素材。
...
1. Windows bottom docking, taskbar avoidance, always-on-top control, and screen footprint; the basic approach has been tested, while final window behavior is deferred until the core game experience is stable.
...
4. Add placeholder NPCs and validate customer seating and player-priority rules.
5. Add music interfaces, local settings, and performance checks.
6. After the core game experience is stable, resume Windows docking, sizing, and always-on-top validation before purchasing cohesive asset packs.
